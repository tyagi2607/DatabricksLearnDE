...
select a.*, b.*
from events_pivot a
INNER JOIN transactions b on a.user = b.user_id;
...
-- MAGIC clickpaths = spark.table("events_pivot").alias("events").join(
-- MAGIC     spark.table("transactions").alias("transactions"),
-- MAGIC     col("events.user") == col("transactions.user_id")
-- MAGIC )

-- COMMAND ----------

-- MAGIC %python
-- MAGIC spark.table("clickpaths")
